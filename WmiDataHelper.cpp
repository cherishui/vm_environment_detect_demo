...
CWmiDataHelper::CWmiDataHelper()
{
	...
	SetComValid(S_OK == CoInitialize(NULL));
...
bool CWmiDataHelper::SetComSecurity()
{
	...
	hRc = CoInitializeSecurity(NULL,
		...
		EOAC_NONE,
		0);
	return ((S_OK != hRc) && (RPC_E_TOO_LATE != hRc)) ? false : true;
}
...
bool CWmiDataHelper::ExecComQuery(IN const wchar_t * pcSqlW,
...
bool CWmiDataHelper::ProcessWbemClassObject(IN ULONG ulRc,
